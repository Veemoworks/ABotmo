...
class ServerPREFIX(commands.Cog):
    ...
    async def rank_prefix(self, ctx, user: discord.Member = None):
        ...
        if not server_settings(False, ctx.guild, "xpenabled"):
            if has_perms:
                embed.set_author(name="This server has XP disabled!", icon_url=ctx.guild.icon.url)
                await ctx.send(embed=embed, delete_after=8)
                return
            await ctx.send("This server has XP disabled!", delete_after=8)
            return
